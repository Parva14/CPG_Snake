...
if __name__ == '__main__':
	...
	while not rospy.is_shutdown():
		...
		if(controller.path != None):

			(trans,rot) = controller.listener.lookupTransform('map', 'base_link', rospy.Time(0))
			(roll, pitch, yaw_c) = euler_from_quaternion (rot)

			if((np.abs((controller.path.poses[(len(controller.path.poses)-1)].pose.position.x) - trans[0]) < 0.1) and (np.abs((controller.path.poses[(len(controller.path.poses)-1)].pose.position.y) - trans[1]) < 0.1)):
			...
			if(reached == False):
				...
				while((np.abs((controller.path.poses[ind+1].pose.position.x) - trans[0]) > 0.015) and (np.abs((controller.path.poses[ind+1].pose.position.y) - trans[1]) > 0.015)):

					if(ind > (len(controller.path.poses)-5)):
						controller.path.poses[ind].pose.position.x = controller.path.poses[(len(controller.path.poses)-1)].pose.position.x
						controller.path.poses[ind].pose.position.y = controller.path.poses[(len(controller.path.poses)-1)].pose.position.y


					(trans,rot) = controller.listener.lookupTransform('map', 'base_link', rospy.Time(0))
					(roll, pitch, yaw_l) = euler_from_quaternion (rot)
					yaw_c = -1*yaw_l
					point = controller.path.poses[ind]
					yaw_d = np.arctan2((controller.path.poses[ind+2].pose.position.x - trans[0]),(controller.path.poses[ind+2].pose.position.y - trans[1])) 
